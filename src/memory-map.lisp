...
(defclass mapped-file ()
  ((filespec :initform nil
   ...
   (delete-policy :initform nil
                  :initarg :delete
                  :reader delete-policy-of
...
(defmethod free-mapped-vector ((vector mapped-vector))
  (prog1
      (munmap vector)
    (with-slots (filespec (deletep delete-policy))
        vector
      (when (and filespec deletep)
...
FOREIGN-TYPE."
  `(progn
     ...
     (defmethod initialize-instance :after ((vector ,name) &key
       ...
       (with-slots (filespec (deletep delete-policy) length mmap-area)
           vector
         (check-arguments (and (integerp length) (plusp length)) (length)
                          "must be a positive integer")
